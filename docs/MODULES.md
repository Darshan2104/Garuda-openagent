...
| `acp/` | `protocol.py` owns the public ACP v1 wire subset: bounded NDJSON JSON-RPC, typed transport failures, and no vendor SDK dependency. `client.py` manages one subprocess with a minimal child environment, process-group lifecycle, deadlines, and bidirectional controller requests. `authority.py`, `normalize.py`, and `broker.py` negotiate and enforce one fail-closed tool owner per family while preserving normalized, redacted session evidence. `adapter.py` supplies the shared `AcpRuntime` conformance path. `fake_agent.py` is the isolated strict-v1 deterministic fixture. `catalog.py` discovers only trusted manifests, binds the accepted absolute executable into launch argv, and never logs in, installs, or reads tokens. `builtin/` contains the user-authenticated Claude Code, Codex, Cursor, and OpenCode manifests. |
