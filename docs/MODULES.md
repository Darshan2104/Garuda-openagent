...
| `sdk/` | `software_agent.py`, `conversation.py` — the library surface, both with a `runtime` selector (native default, unchanged path) plus ACP single-turn runs and held multi-turn harness sessions. |
